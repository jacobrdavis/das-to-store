...
if GENERATE_REFS:
    ...
    logger.info(f'Number of H5 files found: {len(flist)}')

    # Create the reference output directory if it doesn't exist.
    REF_DIR.mkdir(exist_ok=True)

    logger.info(f'Generating individual JSON reference files in {REF_DIR} ...')
...
if COMBINE_REFS:
    ...
    bad_set = {str(p) for p in inconsistent_refs}
